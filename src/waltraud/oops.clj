...
(about #_"Closure"
    ...
    (defn Closure''apply [this, args]
        (let [
            ...
            env (let [x (second fun)] (if (some? x) (assoc env x this) env))
            env (let [x (third  fun)] (if (some? x) (assoc env x args) env))
        ]
...
(about #_"FnExpr"
    (defn FnExpr'macro [& form]
        (let [
            self (first form) ? (symbol? self) self (when ? self) form (if ? (next form) form)
        ]
                ...
                (if (some? s)
                    ...
                    (if (or (not variadic?) (some? etal))
                        (let [
                            etal (or etal (! '&s))
                            form
                                (loop [form (next form) s (seq pars)]
                                    (if (some? s)
                                        (recur (list (cons (! 'let) (cons (list (first s) (list (! 'first) etal) etal (list (! 'next) etal)) form))) (next s))
                                        form
                                    )
                                )
                            form (cons (list (! '&) etal) form)
                        ]
                            (cons (! 'fn*) (if ? (cons self form) form))
                        )
    ...
    (defn FnExpr'parse [form, scope]
        (let [
            ...
            etal
                (loop [etal nil variadic? false s (seq (first form))]
                    (if (some? s)
                        (let [x (first s)]
                            (if (symbol? x)
                                (cond
                                    ...
                                    'else
                                        (if (not variadic?)
                                            (&throw! "forbidden non-variadic parameter " x)
                                            (recur x variadic? (next s))
                                        )
                                )
                                (&throw! "function parameters must be symbols")
                            )
            ...
            scope (if (some? self) (cons self scope) scope)
            scope (if (some? etal) (cons etal scope) scope)
            body (EmbedExpr'parse (cons (! '&do) (next form)), scope)
        ]
            (list &'fn* self etal body)
        )
    )
)
