...
unsigned long strtoul(const char *nptr, char **endptr, unsigned int base)
{
    ...
    while (isxdigit(*nptr) && (value = isdigit(*nptr) ? *nptr-'0' : toupper(*nptr)-'A'+10) < base) {
...
{
    size_t i = vsnprintf(buf, size, fmt, args);
 ...
 * Use snprintf() or scnprintf() in order to avoid buffer overflows.
 */
