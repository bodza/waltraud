...
#define MMPTR(x) x
#else
/* CSRs are stored in subregister slices of CONFIG_CSR_DATA_WIDTH (native endianness),
 * with the least significant slice at the lowest aligned (base) address. */
...
/* CSR subregisters (a.k.a. "simple CSRs") are embedded inside uint32_t aligned locations: */
 ...
 *
 * NOTE: The same optimization considerations apply here as with _csr_rd_buf() above.
 */
...
/* NOTE: the macros' "else" branch is unreachable, no need to be warned about a >= 64bit left shift! */
