...
void sdrrdbuf(int dq)
{
    ...
    for (int p = 0; p < SDRAM_PHY_PHASES; p++) {
        csr_rd_buf_uint8(sdram_dfii_pix_rddata_addr[p], buf, DFII_PIX_DATA_BYTES);
        for (int i = first_byte; i < DFII_PIX_DATA_BYTES; i += step)
            printf("%02x", buf[i]);
    }
    printf("\n");
}
...
void sdrrderr(int count)
{
    ...
    for (int addr = 0; addr < 16; addr++) {
        ...
        cdelay(15);
        for (int p = 0; p < SDRAM_PHY_PHASES; p++)
            csr_rd_buf_uint8(sdram_dfii_pix_rddata_addr[p], prev_data[p], DFII_PIX_DATA_BYTES);

        for (int j = 0; j < count; j++) {
            command_prd(DFII_COMMAND_CAS | DFII_COMMAND_CS | DFII_COMMAND_RDDATA);
            cdelay(15);
            for (int p = 0; p < SDRAM_PHY_PHASES; p++) {
                csr_rd_buf_uint8(sdram_dfii_pix_rddata_addr[p], new_data, DFII_PIX_DATA_BYTES);
                for (int i = 0; i < DFII_PIX_DATA_BYTES; i++) {
...
void sdrwr(unsigned int addr)
{
    ...
    for (int p = 0; p < SDRAM_PHY_PHASES; p++) {
        for (int i = 0; i < DFII_PIX_DATA_BYTES; i++)
            buf[i] = 0x10 * p + i;
        csr_wr_buf_uint8(sdram_dfii_pix_wrdata_addr[p], buf, DFII_PIX_DATA_BYTES);
    }
...
static void write_delay_rst(int module) {
    ...
    for (int i = 0; i < ddrphy_half_sys8x_taps_read(); i++)
...
static int write_level_scan(int *delays, int loops, int show)
{
    ...
    int err_ddrphy_wdly = SDRAM_PHY_DELAYS - ddrphy_half_sys8x_taps_read();
    ...
    for (int i = 0; i < SDRAM_PHY_MODULES; i++) {
        ...
        for (int j = 0; j < err_ddrphy_wdly; j++) {
            int zero_count = 0;
            int one_count = 0;
            int show_iter = show;
    ...
    for (int i = SDRAM_PHY_MODULES-1; i >= 0; i--) {
        if (delays[i] < 0)
            ok = 0;
    }
...
static void write_level_cdly_range(unsigned int *best_error, int *best_cdly, int cdly_start, int cdly_stop, int cdly_step)
{
    int cdly;
    int cdly_actual = 0;
...
int write_level(void)
{
    ...
    /* Center write leveling by varying cdly. Searching through all possible values is slow, but we can
     * use a simple optimization method of iterativly scanning smaller ranges with decreasing step */
    cdly_range_start = 0;
    ...
    while (cdly_range_step > 0) {
        printf("|");
        write_level_cdly_range(&best_error, &best_cdly, cdly_range_start, cdly_range_end, cdly_range_step);
...
static void read_level(int module)
{
    ...
    read_delay_rst(module);
    for (int i = 0; i < (delay_min + delay_max) / 2; i++)
        read_delay_inc(module);
